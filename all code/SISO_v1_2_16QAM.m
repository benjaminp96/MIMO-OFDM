...
x_p = reshape(x_noise,n_fft, length(x_noise+ n_cp - 1)/n_fft);
...
for k = [1:length(rec_sig)]
    if rec_sig(k) ~= t_data(k)
        count(mod_value,i) = count(mod_value,i) + 1;
    end
end

ber(mod_value,i) = count(mod_value,i)/length(t_data);
end
%figure()
semilogy(snr,ber(mod_value,:));
title('BER vs SNR using 16QAM'); legend('16QAM')
