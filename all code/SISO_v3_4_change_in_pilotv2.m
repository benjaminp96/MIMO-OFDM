...
%% reshaping into parallel block streams
X = mod_data;   
...
%%                          CHANNEL
%% Multipath Channel
% delays and attentuation that affects the signal
channel = attenuation;      
...
for i2 = 1:n_fft
H_hat(i2) = Y_blocks(i2,1)./X_block(i2,1);                  % estimating channel using pilot symbols

Y_hat(i2,:) = H_hat(i2) .* X_block(i2,:);                  
end
...
Y_blocks2 = Y_blocks/H_hat_e;                % cancelling out the channel effects with the estimated channel
...
% graph ber of recovered signals
%figure()
